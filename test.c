...
int main() {
	...
	Elf64_Addr origin_entry = e_header->e_entry; //确定入口地址
	...
	for (int i = 0; i < (int) e_header->e_phnum; i++) //e_phnum Program header table entrycount
	{
		...
		/*如果入口地址在段的内存地址中*/
		if ((p_header->p_paddr < origin_entry) &&
			((p_header->p_paddr + p_header->p_filesz) > origin_entry))// p_paddr段在内存中的物理地址 p_filesz该段的结区的总字长
		{
	...
	lseek(fp, (int) e_header->e_shoff - sizeof(e_header) - (int) e_header->e_phnum * sizeof(p_header),
		  SEEK_CUR); //指针跳过程序头部和节区定位到头部
	Elf64_Shdr *s_header;
	for (int i = 0; i < (int) e_header->e_shnum; i++) {
		...
		if ((s_header->sh_addr + s_header->sh_size) == (p_header_vaddr + p_header_size)) {
			Elf64_Off entry_section_offset = s_header->sh_offset;
			Elf64_Xword entry_section_size = s_header->sh_size;
			Elf64_Addr new_entry = s_header->sh_addr + s_header->sh_size; //新的入口地址
		}
	}
}
